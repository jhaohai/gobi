...
use constant {
    OFP_TABLE_MOD_FMT   => "a".OFP_HEADER_LEN." C x3 N",
...
use constant {
    OFP_SWITCH_FEATURES_FMT => "a".OFP_HEADER_LEN." H16 N C C x2 N N",
...
use constant {
    OFP_PORT_STATUS_FMT => "a".OFP_HEADER_LEN." C x7 a".OFP_PORT_LEN,
...
use constant {
    OFP_PORT_MOD_FMT    => "a".OFP_HEADER_LEN." N x4 H12 x2 N N N x4",
    OFP_PORT_MOD_LEN    => 40,
};
...
use constant {
    OFP_FLOW_MOD_FMT    => "a".OFP_HEADER_LEN." H16 H16 C C n n n N N N n x2",
    OFP_FLOW_MOD_LEN    => 56,
};
...
use constant {
    OFP_GROUP_MOD_FMT   => "a.".OFP_HEADER_LEN." n C C N",
...
use constant {
    OFP_PACKET_OUT_FMT  => "a".OFP_HEADER_LEN." N N n x6",
...
use constant {
    OFP_PACKET_IN_FMT   => "a".OFP_HEADER_LEN." N n C C H16 a".OFP_MATCH_LEN,
...
use constant {
    OFP_FLOW_REMOVED_FMT    => "a".OFP_HEADER_LEN." H16 n C C N N n n H16 H16".OFP_MATCH_LEN,
...
sub ofp_switch_features_decode {
    ...
    my($header, $datapath_id, $n_buffers, $n_tables, $auxiliary_id, $capabilities, $reserved) = unpack(OFP_SWITCH_FEATURES_FMT, shift);
    $ofp_switch_features->header($self->ofp_header_decode($header));
...
sub ofp_match_decode {
    ...
    my($type, $length, $buf) = unpack("n n a*", shift);
    my $oxm_fields = substr($buf, 0, $length-4);
    while(length($oxm_fields)) {
        my $ofp_oxm = ofp_oxm->new();
        ...
        $ofp_oxm->oxm_class($oxm_class);
        $ofp_oxm->oxm_field($oxm_field);
        $ofp_oxm->oxm_hasmask($oxm_hasmask);
        $ofp_oxm->oxm_length($oxm_length);
        $ofp_oxm->oxm_value(substr($oxm_fields, OFP_OXM_LEN, $oxm_length));
        $oxm_fields = substr($oxm_fields, OFP_OXM_LEN+$oxm_length);
        print $ofp_oxm->oxm_value."\n";
    }
}
...
sub ofp_packet_in_decode {
    ...
    my($header, $buffer_id, $total_len, $reason, $table_id, $cookie, $buf) = unpack("a".OFP_HEADER_LEN." N n C C H16 a*", shift);
    $ofp_packet_in->header($self->ofp_header_decode($header));
